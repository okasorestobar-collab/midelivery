...
function appendNameQuestionIfNeeded(phoneNumber, conv, reply) {
  ...
  updateClientRecord(phoneNumber, { asked_name_pending: true });
  return text + '\n\nPor cierto, ¿a qué nombre te atiendo? 😊';
}
...
function formatChatwootNote(title, message) {
  ...
  // Se deja al final y lo más discreta posible. Si Chatwoot respeta HTML, <small> la reduce.
  return '**' + title + '**\n' + message + '\n\n<small>_' + getEcuadorShortDateTimeText() + '_</small>';
}
...
function processMessage(userPhone, userMessage) {
  ...
  var systemPrompt = [
    ...
    '- Si el cliente no tiene nombre registrado, DEBES pedir su nombre en el primer o segundo mensaje. Hazlo natural y breve, sin frenar la venta. Ejemplo: Por cierto, ¿a qué nombre te atiendo?',
