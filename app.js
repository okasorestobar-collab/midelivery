...
function detectMessageLanguage(text) {
  ...
  // Español gana si hay señales claras, porque en NJ mucha gente usa palabras como
  // voucher, rent o parking dentro de una conversación en español.
  var spanishScore = 0;
  var englishScore = 0;

  if (/\b(hola|buenas|buenos|buenas tardes|buenas noches|soy|corredor|busco|buscando|quiero|necesito|apartamento|departamento|depa|casa|renta|alquiler|habitaci[oó]n|habitaciones|baño|baños|presupuesto|mudanza|mudarme|visita|verlo|fotos|personas|vivir|pueden|puede|aceptan|mascotas|parqueo|estacionamiento|disponible|direcci[oó]n)\b/i.test(t)) spanishScore += 3;
  if (/[áéíóúñ¿¡]/i.test(t)) spanishScore += 2;
  if (/\b(el|la|los|las|un|una|para|por|con|sin|de|del|que|si|sí)\b/i.test(t)) spanishScore += 1;

  if (/\b(hello|hi|good morning|good afternoon|good evening|i need|i'm looking|i am looking|looking for|do you have|apartment|house|rent|rental|bedroom|bedrooms|bath|bathroom|move|moving|budget|lease|available|photos|pictures|showing|visit|people|occupants|can we|can i|is it okay)\b/i.test(t)) englishScore += 3;
...
function processBrokerStandaloneMessage(brokerPhone, message) {
  ...
  var lang = detectMessageLanguage(message);
  console.log('[Rentas NJ] Mensaje del corredor sin consulta pendiente +' + cleanBroker + ': ' + message);

  // Si el corredor escribe sin una consulta pendiente, no lo tratamos como cliente ni lo mandamos a Claude.
  // Solo confirmamos recepción en el mismo idioma para evitar respuestas largas o fuera de contexto.
  if (lang === 'en') {
    return Promise.resolve('Received. You are registered as the real estate agent for this demo. When Mia needs confirmation about a property, photos, voucher, occupancy, or a showing, she will ask you here.');
  }
  return Promise.resolve('Recibido. Quedas registrado como corredor para esta demo. Cuando Mia necesite confirmar algo de una propiedad, fotos, voucher, ocupación o visita, te consultará por aquí.');
}
...
function sendBrokerConsultRequest(clientPhone, provider, topic, originalMessage) {
  ...
  var exactQuestion = originalMessage || topic || 'consulta inmobiliaria sin detalle';
  var topicText = topic || exactQuestion;
  ...
  updateClientRecord(cleanClient, {
    ...
    real_estate_language: clientLang,
    broker_last_question: exactQuestion,
    broker_last_question_at: new Date().toISOString()
  });

  var lowerQ = String(exactQuestion + ' ' + topicText).toLowerCase();
  var isPhoto = /(foto|fotos|photo|photos|picture|pictures|imagen|imagenes|im[aá]genes)/i.test(lowerQ);
  var isVisit = /(visita|visitar|verlo|verla|showing|tour|appointment|cita)/i.test(lowerQ);

  var msg = '🏠 *Consulta demo inmobiliaria - LinkMarket*\n';
  msg += 'Cliente interesado: +' + cleanClient + (rec.name ? ' (' + rec.name + ')' : '') + '\n';
  msg += 'Idioma del interesado: ' + (clientLang === 'en' ? 'inglés' : 'español') + '\n\n';
  msg += '*Pregunta exacta del interesado:*\n' + exactQuestion + '\n\n';
  if (topicText && topicText !== exactQuestion) {
    msg += '*Contexto detectado por Mia:*\n' + topicText + '\n\n';
  }
  if (isPhoto) {
    msg += 'El interesado pidió fotos. Por ahora, por favor envíale las fotos directamente por WhatsApp al número del interesado.\n\n';
  } else if (isVisit) {
    msg += 'El interesado quiere coordinar una visita. Responde únicamente qué día/hora puedes confirmar o qué horarios tienes disponibles.\n\n';
  } else {
    msg += 'Responde únicamente esa pregunta. No agregues información que el interesado no pidió.\n\n';
  }
  msg += 'Tu respuesta será transmitida por Mia al interesado.';
...
function processBrokerConsultReply(brokerPhone, brokerMessage) {
  ...
  var clientLang = pending.clientLanguage || rec.real_estate_language || detectMessageLanguage(pending.originalMessage || '');

  function deliverBrokerReply(reply) {
    if (conversations[clientPhone] && conversations[clientPhone].history) {
      conversations[clientPhone].history.push({ role: 'assistant', content: 'Corredor confirmó sobre "' + (pending.originalMessage || '') + '": ' + brokerMessage });
    }
  ...
  // En español lo transmitimos de forma casi literal para evitar que Claude cambie la pregunta o invente otro tema.
  if (clientLang !== 'en') {
    var directReply = 'Me confirma el corredor: ' + brokerMessage;
    return deliverBrokerReply(directReply)
      .then(function() {
  ...
  var systemPrompt = [
    'You are Mia from LinkMarket in a New Jersey rentals demo.',
    'The interested client is speaking English. Reply ONLY in English.',
    'Convert the agent reply into a clear WhatsApp message for the interested client.',
    'Do not add unrelated information. Answer only the original question.',
    'Do not promise approval or final availability unless the agent confirmed it.',
    'Original client question: ' + (pending.originalMessage || ''),
    'Agent reply: ' + brokerMessage
  ].join('\n');

  return callClaude([{ role: 'user', content: brokerMessage }], systemPrompt)
    .then(function(reply) {
      return deliverBrokerReply(reply);
    })
    .then(function() {
      console.log('[Rentas NJ] Respuesta del corredor transmitida al cliente +' + clientPhone);
      return true;
    ...
    .catch(function(e) {
      console.error('[Rentas NJ] Error transmitiendo respuesta del corredor: ' + e.message);
      var fallback = 'The agent confirms: ' + brokerMessage;
      return deliverBrokerReply(fallback).then(function(){ return true; }).catch(function(){ return false; });
    });
}
...
function processMessage(userPhone, userMessage) {
  ...
  var detectedLanguage = detectMessageLanguage(userMessage);
  if (/(apartamento|departamento|depa|casa|renta|alquiler|habitaci[oó]n|habitaciones|baño|baños|newark|east orange|irvington|elizabeth|paterson|voucher|visita|fotos|apartment|house|rent|rental|lease|bedroom|bathroom|showing|photos|pictures)/i.test(String(userMessage || ''))) {
    updateClientRecord(userPhone, { real_estate_language: detectedLanguage });
    clientRecord.real_estate_language = detectedLanguage;
  }
  ...
  var systemPrompt = [
    ...
    '- Idioma detectado del ultimo mensaje: ' + (detectedLanguage === 'en' ? 'INGLES' : 'ESPAÑOL') + '.',
    ...
    '- Si el cliente escribe en inglés sobre bienes raíces, respóndele en inglés. Si escribe en español, respóndele en español. El idioma del ULTIMO mensaje manda. No respondas en inglés a un mensaje en español aunque aparezca la palabra voucher, rent o parking.',
    ...
    '- Si el interesado pregunta algo que no está en la ficha, consulta al corredor con el JSON de consultar. En el campo producto escribe la pregunta exacta y corta del interesado, sin agregar otras dudas.',
    '- Si el interesado pregunta cuántas personas pueden vivir, ocupación máxima, occupancy, occupants o si pueden vivir X personas, SIEMPRE consulta al corredor y no respondas otra cosa.',
                ...
                msgs.forEach(function(m) {
                  ...
                  if (m.type === 'text') {
                    ...
                    if (isBrokerNumber(m.from, providers)) {
                      processBrokerStandaloneMessage(m.from, m.text.body)
