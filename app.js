...
function createChatwootMessage(conversationId, message, messageType) {
  ...
  return httpsPost(
    ...
    console.log('[Chatwoot] Mensaje ' + type + ' ID: ' + result.id + ' | conversation_id: ' + conversationId);
    return result;
  });
}
  ...
  analyzeProviderMessage(nombreProveedor, message)
    .then(function(analysis) {
      ...
      if (analysis.tipo === 'cerrado' && clienteActivo) {
        ...
        notifyChatwoot(clienteActivo, analysis.mensaje_cliente, 'outgoing');
        ...
        notifyChatwoot(clienteActivo, msgLink, 'outgoing');
          ...
          if (action.accion === 'pedido_confirmado') {
            ...
            if (metodoPago.toLowerCase().includes('transferencia')) {
              ...
              setTimeout(function() {
                ...
                notifyChatwoot(userPhone, msgTransferencia, 'outgoing');
              }, 2000);
            }
                ...
                msgs.forEach(function(m) {
                  if (m.type === 'text') {
                    ...
                    if (isProviderNumber(m.from, providers)) {
                      ...
                      notifyChatwoot(m.from, m.text.body, 'incoming');
                      processMessage(m.from, m.text.body)
                        .then(function(reply) {
                          sendWhatsApp(m.from, reply)
                            .then(function() {
                              notifyChatwoot(m.from, reply, 'outgoing');
                            });
                        });
                    }
                  }
            ...
            if (isProviderNumber(fromPhone, providers)) {
              ...
              processMessage(fromPhone, msgText)
                .then(function(reply) {
                  ...
                  notifyChatwoot(fromPhone, reply, 'outgoing');
                });
            }
          }
        }
