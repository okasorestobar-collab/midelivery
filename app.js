...
function handleClientWhatsAppMessage(fromPhone, text) {
  ...
  return ensureChatwootConversation(cleanPhone)
    .then(function(conversation) {
      var displayName = getConversationDisplayName(conversation, cleanPhone);

      return createChatwootMessage(conversation.id, text, 'client_public', displayName)
        .catch(function(e) {
          console.error('[Chatwoot] No se pudo crear incoming publico; registro como nota privada: ' + e.message);
  ...
  if (req.method === 'POST' && pathname === '/chatwoot-webhook') {
    ...
    req.on('end', function() {
      try {
        ...
        // WA: texto o Central: texto -> envia texto al WhatsApp y pausa Mia SOLO en este chat.
        // MIA ON          -> devuelve este chat a Mia.
        // MIA OFF         -> pausa Mia en este chat sin enviar mensaje.
        if (isPrivate && /^(WA|CENTRAL)\s*:/i.test(rawText)) {
        ...
        if (!isPrivate && msgType === 'outgoing') {
          ...
          if (/^\*\*Mia\*\*/i.test(rawText)) return;
          if (/^\*\*mia\*\*/i.test(rawText)) return;
          if (/^\*\*sistema\*\*/i.test(rawText)) return;
          if (/^\*\*Central\*\*/i.test(rawText)) return;
          if (/^\*\*central\*\*/i.test(rawText)) return;
          if (/^\*\*adri[aá]n\*\*/i.test(rawText)) return;
