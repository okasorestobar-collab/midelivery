// LinkMarket AI - Node.js puro v3.3
...
// Analiza mensaje del proveedor y decide que hacer
...
function processProviderMessage(providerPhone, message, providers) {
  ...
  analyzeProviderMessage(nombreProveedor, message)
    .then(function(analysis) {
      // Buscar cliente reciente vinculado a este proveedor
      ...
      if (analysis.tipo === 'cerrado' && clienteActivo) {
        ...
        // No hay cliente activo pero igual notificar al dueño (ya se hizo arriba)
      ...
      try {
        ...
        if (s !== -1 && e !== -1) {
          ...
          if (action.accion === 'pedido_confirmado') {
            ...
            // Guardar cliente activo vinculado al proveedor (para respuestas del proveedor)
            if (telefonoProveedor) {
              pendingClientForProvider[telefonoProveedor] = userPhone;
              // Limpiar vínculo despues de 30 minutos
              setTimeout(function() {
...
var server = http.createServer(function(req, res) {
  ...
  if (req.method === 'GET' && pathname === '/') {
    ...
    res.end(JSON.stringify({ status: 'LinkMarket v3.3 operativo' }));
    return;
  }
  ...
  // Webhook de Chatwoot → reenvía mensajes entrantes al bot
  if (req.method === 'POST' && pathname === '/chatwoot-webhook') {
    ...
    req.on('end', function() {
      try {
        ...
        // Solo procesar mensajes entrantes (del cliente), no salientes (del bot)
...
server.listen(PORT, function() {
  console.log('LinkMarket v3.3 corriendo en puerto ' + PORT);
  subscribeWABA();
});
