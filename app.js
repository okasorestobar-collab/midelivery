...
function appendNameQuestionIfNeeded(phoneNumber, conv, reply) {
  ...
  // No mezclar la pregunta del nombre con preguntas operativas del pedido.
  ...
  var operationalQuestion = /(retiro|retiras|local|domicilio|env[ií]o|delivery|direcci[oó]n|a d[oó]nde|d[oó]nde te lo|forma de pago|m[eé]todo de pago|efectivo|transferencia|tarjeta|total|resumen|salsa|salsas)/i.test(text);
...
var server = http.createServer(function(req, res) {
  ...
  if (req.method === 'GET' && pathname === '/') {
    ...
    res.end(JSON.stringify({ status: 'LinkMarket v3.5-v18-final operativo' }));
    return;
  }
...
server.listen(PORT, function() {
  console.log('LinkMarket v3.5-v18-final corriendo en puerto ' + PORT);
  subscribeWABA();
});
