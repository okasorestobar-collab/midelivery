// LinkMarket AI - Node.js puro v3.2
...
// Manejo de mensajes de PROVEEDORES
function processProviderMessage(providerPhone, message, providers) {
  ...
  // Notificar al dueño con el mensaje del proveedor
  if (OWNER_NUMBER) {
  ...
  var respuesta = '¡Hola! 👋 Soy Mia, el asistente de LinkMarket.\n\n';
  respuesta += 'Recibí tu mensaje. Para temas administrativos o consultas, te comunico con el equipo de LinkMarket.\n\n';
...
function processMessage(userPhone, userMessage) {
  ...
  if (isProviderNumber(userPhone, providers)) {
    return processProviderMessage(userPhone, userMessage, providers)
      .then(function(reply) { sendWhatsApp(userPhone, reply); });
  }
  ...
  // Limpiar historial si el ultimo pedido fue hace mas de 2 horas (evita contaminacion)
  ...
  var systemPrompt = [
    'Eres "Mia", asistente de delivery de LinkMarket. Tono: amable, directo y conciso. Mensajes cortos.',
    '',
    'CATALOGO:\n' + catalogText,
    '',
    ...
    '- Delivery FUERA de Plaza Volare: informa que debes consultar el costo al restaurante.',
    '',
    'PAGOS ACEPTADOS:',
    ...
    '5. Muestra resumen: items + envases + delivery = total',
    '6. Pregunta metodo de pago (efectivo / transferencia / tarjeta)',
    '7. Solo si elige transferencia, da los datos bancarios',
    '8. Confirma con el JSON',
    '',
    'DESCRIPCIONES: Breves y apetitosas. Max 2 lineas. Solo si el cliente pregunta.',
    '',
    'CONSULTAS AL PROVEEDOR:',
    '- Si piden modificacion (sin maní, sin picante, etc.): responde "Déjame verificar con el restaurante un momento 🔍" y en el siguiente turno indica que lo estas gestionando.',
      ...
      try {
        ...
        if (s !== -1 && e !== -1) {
          ...
          if (action.accion === 'pedido_confirmado') {
            ...
            // Mensaje PROVEEDOR (incluye forma de pago porque ellos hacen la entrega)
            ...
            // Mensaje LOGISTICA (solo si es diferente al proveedor)
            ...
            // Datos de transferencia AL CLIENTE solo si eligio ese metodo
            if (metodoPago.toLowerCase().includes('transferencia')) {
            ...
            // Marcar tiempo del pedido y limpiar historial
...
var server = http.createServer(function(req, res) {
  ...
  if (req.method === 'GET' && pathname === '/') {
    ...
    res.end(JSON.stringify({ status: 'LinkMarket v3.2 operativo' }));
    return;
  }
...
server.listen(PORT, function() {
  console.log('LinkMarket v3.2 corriendo en puerto ' + PORT);
  subscribeWABA();
});
