...
var humanTakeoverUntil = {};
var HUMAN_TAKEOVER_MINUTES = parseInt(process.env.HUMAN_TAKEOVER_MINUTES || '60', 10);

function isHumanTakeoverActive(phone) {
  var cleanPhone = normalizePhone(phone);
  return humanTakeoverUntil[cleanPhone] && humanTakeoverUntil[cleanPhone] > Date.now();
}

function pauseMiaForHuman(phone) {
  var cleanPhone = normalizePhone(phone);
  humanTakeoverUntil[cleanPhone] = Date.now() + HUMAN_TAKEOVER_MINUTES * 60 * 1000;
  console.log('[Control humano] Mia pausada para ' + cleanPhone + ' por ' + HUMAN_TAKEOVER_MINUTES + ' minutos');
}
...
function createChatwootMessage(conversationId, message, messageType, isPrivate, contentPrefix) {
  var type = messageType || 'outgoing';
  var content = message;
  ...
  return httpsPost(
    ...
    if (!result || !result.id) {
      console.error('[Chatwoot] Error creando mensaje. Respuesta: ' + JSON.stringify(result));
      throw new Error('Chatwoot no devolvio message.id');
    }

    console.log('[Chatwoot] Mensaje ' + type + (isPrivate ? ' privado' : '') + ' ID: ' + result.id + ' | conversation_id: ' + conversationId);
    return result;
  });
}

function notifyChatwoot(toPhone, message, messageType, options) {
...
function notifyChatwootIncoming(toPhone, message) {
  return notifyChatwoot(toPhone, message, 'incoming', { private: false });
}

function notifyChatwootBotSent(toPhone, message) {
  return notifyChatwoot(toPhone, message, 'outgoing', {
    private: true,
    prefix: '[MIA ENVIADO POR WHATSAPP]'
  });
}
  ...
  analyzeProviderMessage(nombreProveedor, message)
    .then(function(analysis) {
      ...
      if (analysis.tipo === 'cerrado' && clienteActivo) {
        ...
        notifyChatwootBotSent(clienteActivo, analysis.mensaje_cliente);
        ...
        notifyChatwootBotSent(clienteActivo, msgLink);
...
function processMessage(userPhone, userMessage) {
  ...
  var systemPrompt = [
    ...
    '- En tu bienvenida preséntate claramente como Mia de LinkMarket. Ejemplo: "Hola, soy Mia de LinkMarket" y pregunta qué necesita el cliente (comida, mandados, etc).',
          ...
          if (action.accion === 'pedido_confirmado') {
            ...
            if (metodoPago.toLowerCase().includes('transferencia')) {
              ...
              setTimeout(function() {
                ...
                notifyChatwootBotSent(userPhone, msgTransferencia);
              }, 2000);
            }
                ...
                msgs.forEach(function(m) {
                  if (m.type === 'text') {
                    ...
                    if (isProviderNumber(m.from, providers)) {
                      ...
                      notifyChatwootIncoming(m.from, m.text.body);

                      if (isHumanTakeoverActive(m.from)) {
                        console.log('[Control humano] Mensaje recibido de ' + m.from + ' registrado en Chatwoot. Mia no responde por pausa humana.');
                        return;
                      }
                      ...
                      processMessage(m.from, m.text.body)
                        .then(function(reply) {
                          sendWhatsApp(m.from, reply)
                            .then(function() {
                              notifyChatwootBotSent(m.from, reply);
                            });
                        });
                    }
                  }
  ...
  if (req.method === 'POST' && pathname === '/chatwoot-webhook') {
    ...
    req.on('end', function() {
      try {
        ...
        if (cw.event !== 'message_created' || !cw.content) {
          return;
        }

        var msgText = String(cw.content || '');
        var msgType = cw.message_type;
        var isPrivate = !!cw.private;

        // Ignorar notas privadas creadas por el propio bot para registrar lo que Mia ya envió por WhatsApp.
        if (msgText.indexOf('[MIA ENVIADO POR WHATSAPP]') === 0) {
          return;
        }
        ...
        var senderPhone =
          ...
          cw.conversation.meta.sender &&
          cw.conversation.meta.sender.phone_number;

        var contactPhone =
          senderPhone ||
          (cw.contact && cw.contact.phone_number) ||
          (cw.conversation && cw.conversation.contact && cw.conversation.contact.phone_number) ||
          '';

        if (!contactPhone) {
          console.log('[Chatwoot] Webhook sin telefono de contacto. Payload parcial: ' + JSON.stringify(cw).substring(0, 500));
          return;
        }

        var fromPhone = normalizePhone(contactPhone);

        // Intervención humana:
        // 1) Si respondes normalmente desde Chatwoot, se envía por WhatsApp desde Railway.
        // 2) Si Chatwoot bloquea la caja por ventana 24h, usa Nota privada escribiendo: WA: tu mensaje
        // En ambos casos Mia queda pausada temporalmente para ese cliente.
        if (msgType === 'outgoing' && fromPhone && msgText) {
          if (isPrivate) {
            var lower = msgText.toLowerCase();
            if (lower.indexOf('wa:') !== 0 && lower.indexOf('whatsapp:') !== 0) {
              return;
            }

            msgText = msgText.replace(/^wa:/i, '').replace(/^whatsapp:/i, '').trim();
          ...
          pauseMiaForHuman(fromPhone);
          console.log('[Chatwoot] Enviando intervención humana a WhatsApp +' + fromPhone + ': ' + msgText);
          sendWhatsApp(fromPhone, msgText);
        }
      } catch(e) {
        console.error('Error chatwoot-webhook: ' + e.message);
      }
    });
    return;
  }
